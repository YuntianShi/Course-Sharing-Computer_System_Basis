...
int divpwr2(int x, int n) {
  int a = x >> 31;//取出符号位
  int b = ((~0) + (1 << n)) & a;//设置判据，若x为正数，则直接右移n位，若x为负数，则需加上2的n次方-1后，再右移n位
  return ((x + b) >> n);
}
...
int bitCount(int x) {
  ...
  a = a + (x & k);
  x = x >> 1;
  a = a + (x & k);//以4位大小划分成8块，对每块中1的位数分别进行计算（和0x11111111进行按位与并移动1位，重复4次，最后一次不移动），求出8块中的总和，得到结果
  return ((a & 0xf) + ((a >> 4) & 0xf) + ((a >> 8) & 0xf) + ((a >> 12) & 0xf) + ((a >> 16) & 0xf) + ((a >> 20) & 0xf) + ((a >> 24) & 0xf) + ((a >> 28) & 0xf));
}
